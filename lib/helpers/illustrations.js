 ...
 *
 * @param {string} base64 - A data URI string e.g. "data:image/png;base64,..."
 * @returns {string} - jsPDF-compatible format string e.g. 'JPEG', 'PNG', 'WEBP'
 */
  ...
  doc.autoTable({
    ...
    didDrawCell: function (data) {
      if (data.section === 'body' && data.cell.imgObj) {
        ...
        var str = obj.base64
        // Detect image format from the data URI rather than assuming JPEG.
